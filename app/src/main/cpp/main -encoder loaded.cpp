...
void CameraMixedRealityApp::InitializeONNX() {
    ...
    if (!ort) {
        ALOGE("Failed to get ORT API");
        onnx_status_message_ = "Failed to get ONNX Runtime API.";
        return;
    }
    ...
    onnx_status_message_ += "\nONNX Runtime Version: " + (ort_version ? std::string(ort_version) : "unknown");
    ...
    if (status) {
        ...
        ALOGE("ONNX Env Failed: %s", errMsg ? errMsg : "unknown");
        onnx_status_message_ = "ONNX init failed: " + (errMsg ? std::string(errMsg) : "unknown");
        ort->ReleaseStatus(status);
        return;
    }

    // Session options (enable GPU if available)
    OrtSessionOptions* session_options;
    ort->CreateSessionOptions(&session_options);
    ort->SetIntraOpNumThreads(session_options, 1); // Adjust threads as needed
    ...
    OrtSession* encoder_session = nullptr;
    status = ort->CreateSession(env, encoder_path.c_str(), session_options, &encoder_session);
    if (status) {
        const char* errMsg = ort->GetErrorMessage(status);
        ALOGE("Encoder Load Failed: %s", errMsg ? errMsg : "unknown");
        onnx_status_message_ += "\nEncoder Load Failed: " + (errMsg ? std::string(errMsg) : "unknown");
        ort->ReleaseStatus(status);
    } else {
        ALOGI("Encoder Model Loaded!");
    ...
    OrtSession* decoder_session = nullptr;
    status = ort->CreateSession(env, decoder_path.c_str(), session_options, &decoder_session);
    if (status) {
        const char* errMsg = ort->GetErrorMessage(status);
        ALOGE("Decoder Load Failed: %s", errMsg ? errMsg : "unknown");
        onnx_status_message_ += "\nDecoder Load Failed: " + (errMsg ? std::string(errMsg) : "unknown");
        ort->ReleaseStatus(status);
    } else {
        ALOGI("Decoder Model Loaded!");
        onnx_status_message_ += "\nDecoder Model Loaded!";
    }

    // Store sessions for later use
    this->encoder_session_ = encoder_session;
    this->decoder_session_ = decoder_session;
