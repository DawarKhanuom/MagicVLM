...
static RunTiming g_last_timing;  // filled by InitializeONNX() for the last run
//..............................................
...
private:
    //--------------------for timeanalysis ------------------
    // --- Perf test helpers ---
    void RunBatchTimeTest(const std::string& dir,
                          const std::string& out_path,
                          const std::string& question,
                          bool warmup);
    void RunModelLoadTest(int repeats, const std::string& out_path);

// record how many tokens were generated last run (for tokens/sec)
...
//................................time analysis-----------------start ---------------
        if (ImGui::Button("Run time test (images -> timetest.txt)")) {
            // Disable auto-TTS during the batch
            bool prev_speak = speak_auto_;
            speak_auto_ = false;

            std::thread([this, prev_speak]() {
                const char* images_dir =
                        "/storage/emulated/0/Android/data/com.magicleap.capi.sample.camera_mixed_reality/files/images";
                const char* out_path =
                        "/storage/emulated/0/Android/data/com.magicleap.capi.sample.camera_mixed_reality/files/timetest.txt";

                // Ensure model is loaded once
                if (!EnsureVLMLoaded()) {
                    onnx_status_message_ = "VLM not loaded: timing test aborted.";
                    speak_auto_ = prev_speak;
                    return;
                }

                // Collect image files
                std::vector<std::string> files;
                if (DIR* d = opendir(images_dir)) {
                    while (dirent* e = readdir(d)) {
                        ...
                        if (lower.size() >= 4 &&
                            ...
                            files.emplace_back(std::string(images_dir) + "/" + name);
                        }
                    }
                    closedir(d);
                } else {
                    onnx_status_message_ = "Could not open images dir (check path/permissions).";
                    speak_auto_ = prev_speak;
                    return;
                }
                ...
                if (files.empty()) {
                    onnx_status_message_ = "No images found in /files/images.";
                    speak_auto_ = prev_speak;
                    return;
                }

                // Open output once; write header if file is empty
                FILE* f = fopen(out_path, "a+");
                if (!f) {
                    onnx_status_message_ = "Cannot open timetest.txt for writing.";
                    speak_auto_ = prev_speak;
                    return;
                }
                fseek(f, 0, SEEK_END);
                long sz = ftell(f);
                if (sz == 0) {
                    fprintf(f, "image,load_ms,enc_ms,ttft_ms,decode_ms,tokens,toks_per_s,total_ms,online_ms\n");
                    fflush(f);
                }

                const std::string q = "What is in the image?"; // fixed question for fair timing

                size_t wrote = 0;
                for (const auto& path : files) {
                    // Synchronous call; InitializeONNX updates g_last_timing
                    InitializeONNX(path, q);

                    // ONLINE = time until first token (image load + encoder + TTFT)
                    double online_ms = g_last_timing.load_ms
                                       + g_last_timing.enc_ms
                                       + g_last_timing.ttft_ms;

                    // basename for CSV
                    const char* slash = strrchr(path.c_str(), '/');
                    const char* base  = slash ? slash + 1 : path.c_str();

                    fprintf(f, "%s,%.3f,%.3f,%.3f,%.3f,%d,%.3f,%.3f,%.3f\n",
                            base,
                            g_last_timing.load_ms,
                            g_last_timing.enc_ms,
                            g_last_timing.ttft_ms,
                            g_last_timing.dec_ms,
                            g_last_timing.tokens,
                            g_last_timing.toks_per_s,
                ...
                onnx_status_message_ = "Timing test complete. Wrote " + std::to_string(wrote)
                                       + " rows to timetest.txt";
                speak_auto_ = prev_speak; // restore TTS preference
            }).detach();
        }
//............................
        ImGui::Separator();
        if (ImGui::CollapsingHeader("Performance Tests", ImGuiTreeNodeFlags_DefaultOpen)) {
            if (ImGui::Button("Run image batch time test")) {
                const std::string imgdir = "/storage/emulated/0/Android/data/com.magicleap.capi.sample.camera_mixed_reality/files/images";
                const std::string out    = "/storage/emulated/0/Android/data/com.magicleap.capi.sample.camera_mixed_reality/files/timetest.txt";
                const std::string q      = "What is in the image?";
                std::thread([this, imgdir, out, q]() {
                    RunBatchTimeTest(imgdir, out, q, /*warmup=*/true);
                }).detach();
            }
            ImGui::SameLine();
            if (ImGui::Button("Measure model load time")) {
                const std::string out    = "/storage/emulated/0/Android/data/com.magicleap.capi.sample.camera_mixed_reality/files/timetest.txt";
                std::thread([this, out]() {
                    RunModelLoadTest(/*repeats=*/3, out);
                }).detach();
            }
        }
...
void CameraMixedRealityApp::InitializeONNX(const std::string& image_path,
    ...
    auto t0 = now(); // start BEFORE LoadImageCHW224F32
...
//    }
    ...
    int W = 0, H = 0;
// thiswas changed ----   if (!LoadImageCHW224F32(image_path, pixel_values, W, H)) {
    if (!LoadImageCHW224F32(use_path, pixel_values, W, H)) {
        onnx_status_message_ = "Failed to load image.";
        clear_busy();
        return;
    }
    //...............time ................................
    auto t1 = now(); // after load + preprocess
    ...
    //...................................time .................
    auto t2 = now();                 // after encoder
    auto t_dec_start = t2;           // decoder window
    bool saw_first_token = false;
    Clock::time_point t_first_token = t2;  // will be set when first token arrives
...
//...........................................time ...........................
        if (!saw_first_token) { t_first_token = now(); saw_first_token = true; }
...
//................................................ timing ......................................
    ...
    // >>> TIMING: after the loop, compute metrics and stash them
    auto t_dec_end = now();

    int tokens = (int)generated.size();
    if (tokens > 0 && generated.back() == eos_id) --tokens; // exclude EOS

    double load_ms   = ms(t1 - t0);
    double enc_ms    = ms(t2 - t1);
    double ttft_ms   = ms(t_first_token - t2);     // time to first token
    double dec_ms    = ms(t_dec_end - t_dec_start);
    double total_ms  = ms(t_dec_end - t0);         // end-to-end (image → last token)
    double toks_per_s = (dec_ms > 0.0) ? (tokens / (dec_ms / 1000.0)) : 0.0;

    g_last_timing = { load_ms, enc_ms, ttft_ms, dec_ms, total_ms, tokens, toks_per_s };
//................................................ timing ......................................
...
// Persist Q/A for UI + TTS
    ...
    //..........................time --------------------------
// Persist Q/A for UI + TTS
    last_question_text_ = TrimCollapse(question_text);
    last_answer_text_   = answer;
    last_token_count_   = static_cast<int>(generated.size());       // <--- add this line
...
//=========================time analysis -----------------------------------------------
// Simple helper: list image files in a directory
static std::vector<std::string> ListImagesInDir(const std::string& dir) {
    ...
    while (auto* e = readdir(d)) {
        ...
        bool is_img = (low.rfind(".jpg")  != std::string::npos) ||
                      (low.rfind(".jpeg") != std::string::npos) ||
                      (low.rfind(".png")  != std::string::npos);
        if (is_img) out.push_back(dir + "/" + name);
    }
    closedir(d);
#endif
...
void CameraMixedRealityApp::RunBatchTimeTest(const std::string& dir,
                                             const std::string& out_path,
                                             const std::string& question,
                                             bool warmup) {
    StopListening();  // avoid mic while testing

    onnx_status_message_ = "Batch time test: scanning images…\n";
    auto files = ListImagesInDir(dir);
    if (files.empty()) {
        onnx_status_message_ += "No images found in: " + dir + "\n";
        return;
    }

    // Optional cap to ~100 as you requested
    if (files.size() > 100) files.resize(100);

    // Make sure model is loaded exactly once
    const auto t_load0 = std::chrono::steady_clock::now();
    if (!EnsureVLMLoaded()) {
        onnx_status_message_ += "Failed to load VLM.\n";
        return;
    }
    const auto t_load1 = std::chrono::steady_clock::now();
    const double load_ms = std::chrono::duration<double, std::milli>(t_load1 - t_load0).count();

    // Open results file (overwrite each run; change to "a" to append if you prefer)
    FILE* f = fopen(out_path.c_str(), "w");
    if (!f) {
        onnx_status_message_ += "Cannot open output file: " + out_path + "\n";
        return;
    }

    fprintf(f, "VLM load once: %.3f ms\n", load_ms);
    fprintf(f, "Images dir: %s\n", dir.c_str());
    fprintf(f, "Question: %s\n", question.c_str());
    fprintf(f, "File\tms_total\tgen_tokens\tchars\ttokens_per_s\n");

    std::vector<double> times_ms;
    times_ms.reserve(files.size());

    // Warm-up (optional): run the first image but don't record; JIT/caches stabilize
    size_t start_i = 0;
    if (warmup && !files.empty()) {
        onnx_status_message_ += "Warm-up on first image…\n";
        InitializeONNX(files[0], question);
        start_i = 1;
    }

    for (size_t i = start_i; i < files.size(); ++i) {
        const std::string& p = files[i];

        const auto t0 = std::chrono::steady_clock::now();
        InitializeONNX(p, question);            // synchronous run (your function blocks)
        const auto t1 = std::chrono::steady_clock::now();

        const double ms = std::chrono::duration<double, std::milli>(t1 - t0).count();
        times_ms.push_back(ms);

        const int tokens = last_token_count_;
        const int chars  = (int)last_answer_text_.size();
        const double tps = (ms > 0.0) ? (tokens / (ms / 1000.0)) : 0.0;

        fprintf(f, "%s\t%.3f\t%d\t%d\t%.2f\n", p.c_str(), ms, tokens, chars, tps);
        fflush(f);
#ifndef ML_WINDOWS
        int fd = fileno(f);
        if (fd >= 0) fsync(fd);
        ...
        // also reflect progress in the GUI
        onnx_status_message_ = "Processed " + std::to_string(i + 1 - start_i) + " / " +
                               std::to_string(files.size() - start_i) + "\n";
    }

    // Stats
    double sum = 0.0;
    for (double v : times_ms) sum += v;
    double avg = (times_ms.empty() ? 0.0 : sum / times_ms.size());
    std::vector<double> sorted = times_ms;
    std::sort(sorted.begin(), sorted.end());
    auto pct = [&](double p)->double{
        if (sorted.empty()) return 0.0;
        double idx = (p/100.0) * (sorted.size()-1);
        size_t i0 = (size_t)idx, i1 = std::min(sorted.size()-1, i0+1);
        double t = idx - i0;
        return sorted[i0]*(1.0-t) + sorted[i1]*t;
    };

    fprintf(f, "N=%zu, avg=%.3f ms, median=%.3f ms, p90=%.3f ms, p95=%.3f ms, min=%.3f ms, max=%.3f ms\n",
            times_ms.size(), avg, pct(50), pct(90), pct(95),
            sorted.empty()?0.0:sorted.front(),
            sorted.empty()?0.0:sorted.back());
    fclose(f);

    onnx_status_message_ += "Time test written to:\n" + out_path + "\n";
}

void CameraMixedRealityApp::RunModelLoadTest(int repeats, const std::string& out_path) {
    StopListening();

    if (repeats < 1) repeats = 1;
    FILE* f = fopen(out_path.c_str(), "a");   // append after batch test
    if (!f) {
        onnx_status_message_ = "Cannot open " + out_path + " for model-load test.\n";
        return;
    }
    fprintf(f, "\n=== Model load time test (%d repeats) ===\n", repeats);

    std::vector<double> ms;
    ms.reserve(repeats);
    for (int i=0; i<repeats; ++i) {
        UnloadVLM(); // ensure a cold start
        const auto t0 = std::chrono::steady_clock::now();
        bool ok = EnsureVLMLoaded();
        const auto t1 = std::chrono::steady_clock::now();
        double dt = std::chrono::duration<double, std::milli>(t1 - t0).count();
        ms.push_back(dt);
        fprintf(f, "Load #%d: %.3f ms (%s)\n", i+1, dt, ok?"ok":"FAIL");
        fflush(f);
#ifndef ML_WINDOWS
        int fd = fileno(f);
        if (fd >= 0) fsync(fd);
#endif
        if (!ok) { onnx_status_message_ = "Load failed during test."; break; }
    }
    ...
    if (!ms.empty()) {
        ...
        auto val=[&](double p){
            ...
            size_t i0=(size_t)idx,i1=std::min(s.size()-1,i0+1); double t=idx-i0;
            return s[i0]*(1.0-t)+s[i1]*t;
        };
        fprintf(f, "Load avg=%.3f ms, median=%.3f ms, p90=%.3f ms\n", sum/ms.size(), val(50), val(90));
    }
    fclose(f);

    onnx_status_message_ += "Model-load test appended to:\n" + out_path + "\n";
}
