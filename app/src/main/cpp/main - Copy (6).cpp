...
void CameraMixedRealityApp::InitializeONNX() {
    ...
    OrtStatus* status = ort_->CreateEnv(ORT_LOGGING_LEVEL_WARNING, "ML2App", &env);
    if (status) {
        const char* errMsg = ort_->GetErrorMessage(status);
        onnx_status_message_ = "ONNX env failed: " + std::string(errMsg ? errMsg : "unknown");
        ALOGE("%s", onnx_status_message_.c_str());
        ort_->ReleaseStatus(status);
        return;
    }
    ...
    if (status) {
        const char* errMsg = ort_->GetErrorMessage(status);
        onnx_status_message_ += "\nEncoder load failed: " + std::string(errMsg ? errMsg : "unknown");
        ALOGE("%s", onnx_status_message_.c_str());
        ort_->ReleaseStatus(status);
    } else {
    ...
    if (status) {
        const char* errMsg = ort_->GetErrorMessage(status);
        onnx_status_message_ += "\nDecoder load failed: " + std::string(errMsg ? errMsg : "unknown");
        ALOGE("%s", onnx_status_message_.c_str());
        ort_->ReleaseStatus(status);
    } else {
    ...
    // --- 6. Test Decoder with Hardcoded Input ---
    if (decoder_session_) {
        // Example input tokens (replace with your actual token IDs)
        std::vector<int64_t> input_ids = {50256}; // BOS token for GPT-2
        std::vector<int64_t> input_shape = {1, static_cast<int64_t>(input_ids.size())};

        OrtMemoryInfo* memory_info = nullptr;
        ort_->CreateCpuMemoryInfo(OrtArenaAllocator, OrtMemTypeDefault, &memory_info);

        OrtValue* input_tensor = nullptr;
        ort_->CreateTensorWithDataAsOrtValue(
                memory_info,
                input_ids.data(),
        ...
        if (!status) {
            ...
            const char* errMsg = ort_->GetErrorMessage(status);
            onnx_status_message_ += "\nDecoder test failed: " + std::string(errMsg ? errMsg : "unknown");
            ALOGE("%s", onnx_status_message_.c_str());
            ort_->ReleaseStatus(status);
        }

        ort_->ReleaseValue(input_tensor);
        ort_->ReleaseMemoryInfo(memory_info);
    }
