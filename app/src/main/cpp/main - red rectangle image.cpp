...
#ifdef ML_LUMIN
    // Transparent clear so compositor shows real world instead of black
    glClearColor(0.f, 0.f, 0.f, 0.f);
    glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
    // If you manage depth, also:
    // glClearDepthf(1.0f);
#endif
}
...
void CameraMixedRealityApp::UpdateGazeButtonAndTrigger(float dt_seconds) {
    ...
    if (auto_dwell_snap_ && gaze_hits_button) {
        ...
        if (dwell_timer_ms_ >= dwell_ms_needed_) {
            ...
            // Optional: record click-local (legacy path). World-rect crop is preferred now.
...
//...................... end of button trigger................................
 ...
 void CameraMixedRealityApp::SetupSceneNodes() {
     ...
     if (!aim_pad_node_) {
         ...
         pad_rc->SetVisible(true); // keep visible (very faint) so you can debug. Set to false to hide completely.
...
void CameraMixedRealityApp::UpdateWorldCropFrame() {
    if (!crop_frame_node_ || !aim_pad_node_) return;

    // map normalized crop center/size [0..1] → pad local meters
    const glm::vec2 halfPad = {
            fabsf(aim_pad_node_->GetLocalScale().x) * 0.5f,
            fabsf(aim_pad_node_->GetLocalScale().y) * 0.5f
    };

    const float cx = (crop_norm_center_.x - 0.5f) * 2.0f * halfPad.x;
    const float cy = (0.5f - crop_norm_center_.y) * 2.0f * halfPad.y; // +Y up
    const float w  = crop_norm_size_.x * 2.0f * halfPad.x;
    const float h  = crop_norm_size_.y * 2.0f * halfPad.y;

    // thickness of the frame (meters)
    const float th = 0.004f;

    // Edges: 0=L, 1=T, 2=R, 3=B
    // Left
    crop_edges_[0]->SetLocalTranslation(glm::vec3(cx - 0.5f*w, cy, 0.001f));
    crop_edges_[0]->SetLocalScale      (glm::vec3(th, h, 0.001f));
    // Top
    crop_edges_[1]->SetLocalTranslation(glm::vec3(cx, cy + 0.5f*h, 0.001f));
    crop_edges_[1]->SetLocalScale      (glm::vec3(w, th, 0.001f));
    // Right
    crop_edges_[2]->SetLocalTranslation(glm::vec3(cx + 0.5f*w, cy, 0.001f));
    crop_edges_[2]->SetLocalScale      (glm::vec3(th, h, 0.001f));
    // Bottom
    crop_edges_[3]->SetLocalTranslation(glm::vec3(cx, cy - 0.5f*h, 0.001f));
    crop_edges_[3]->SetLocalScale      (glm::vec3(w, th, 0.001f));
}
...
void CameraMixedRealityApp::UpdateGui() {
    ...
    ImGui::PushStyleColor(ImGuiCol_WindowBg, ImVec4(0,0,0,0.15f));
    ...
    ImVec2 mainSize(std::min(screen.x * 0.40f, 640.0f),
                    std::min(screen.y * 0.86f, 840.0f));
...
#ifdef ML_LUMIN
        // Preview + crop (kept in sync with the world red rectangle)
        if (!last_captured_image_.empty()) {
            if (last_photo_loaded_path_ != last_captured_image_) {
                ...
                // keep existing crop_norm_* (user might have positioned it in world)
            }

            if (last_photo_tex_) {
                const float maxW = 520.0f;
                ...
                if (ImGui::Button("Crop selection now")) {
                    ...
                    if (W>0 && H>0) {
                        ...
                        if (cw>8 && ch>8) {
                            ...
                            CropAndSaveSquareJPEG(last_captured_image_, crop_path, cx, cy, std::min(cw, ch));
                            PersistLastImageAlias(crop_path);
                            last_photo_loaded_path_.clear();
                        }
                    }
                }
            }
...
MLResult CameraMixedRealityApp::SetupCamera() {
    ...
    // In SetupCamera(), before MLCameraConnect(...)
//    camera_connect_context.mr_info.blend_type = MLCameraMRBlendType_Alpha;
