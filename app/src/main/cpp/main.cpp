...
void CameraMixedRealityApp::UpdateGui() {
    ...
    if (gui.BeginDialog("Image + Voice Q&A", &app_running,
        ...
        if (ImGui::Button("Capture + Ask (use text below)")) {
        ...
        if (!last_captured_image_.empty()) {
            if (ImGui::Button("🎙️  Ask by voice (listen now)")) {
        ...
        if (ImGui::Button("Ask (text)")) {
            if (!last_captured_image_.empty() && question_buf_[0] != '\0') {
                InitializeONNX(last_captured_image_, std::string(question_buf_));
            } else {
...
void CameraMixedRealityApp::OnImageAvailable(const MLCameraOutput *output,
    ...
    if (f) {
        ...
        // Only now that the file exists, publish it
        this_app->last_captured_image_ = output_filename;

        if (this_app->pending_voice_question_) {
            // user chose "Capture photo + then ask by voice"
            this_app->pending_voice_question_ = false;
            this_app->StartListening();
        } else if (this_app->send_to_vlm_after_capture_) {
            // user chose "Capture + Ask (use text below)"
            ...
            this_app->InitializeONNX(output_filename, q);
            this_app->typed_question_.clear();            // reset snapshot
        }
    } else {
...
void CameraMixedRealityApp::InitializeONNX(const std::string& image_path,
    ...
    if (busy.test_and_set()) { onnx_status_message_ = "ONNX is already running…"; return; }
    ...
    auto ReadFile = [](const std::string& p, std::string& out)->bool{
        ...
        fseek(f, 0, SEEK_END); long n = ftell(f); if(n < 0){ fclose(f); return false; }
        fseek(f, 0, SEEK_SET); out.resize((size_t)n);
    ...
    auto LoadGpt2VocabIdToTokenLocal = [&](const std::string& vocab_path, std::vector<std::string>& id2tok)->bool {
        std::string s;
        if (!ReadFile(vocab_path, s)) return false;
        ...
        id2tok.clear(); id2tok.resize((size_t)max_id + 1);
        for (auto it = j.begin(); it != j.end(); ++it) {
            if (!it.value().is_number_integer()) continue;
            int64_t id = it.value().get<int64_t>();
    ...
    auto SimpleDecodeGpt2Local = [&](const std::vector<int64_t>& ids, const std::vector<std::string>& id2tok)->std::string {
        std::string out; out.reserve(ids.size()*3);
        for (auto id : ids) {
        ...
        for (size_t i=1; i<out.size();) { if (out[i]==' ' && out[i-1]==' ') out.erase(i,1); else ++i; }
        return out;
    };
    auto LoadImageCHW224F32 = [&](const std::string& path, std::vector<float>& out, int& W, int& H)->bool {
        int w=0, h=0, c=0;
        ...
        auto samp = [&](float x, float y, int chn)->float {
            ...
            int idx = (iy*w + ix)*3 + chn; return (float)img[idx] / 255.0f;
        };
        for (int oy=0; oy<outH; ++oy) for (int ox=0; ox<outW; ++ox) {
                ...
                int idx = oy*outW + ox;
                out[0*outH*outW + idx] = r; out[1*outH*outW + idx] = g; out[2*outH*outW + idx] = b;
            }
        stbi_image_free(img); W = outW; H = outH; return true;
    };
    auto TokenizeQuestion = [&](const std::string& question, const std::vector<std::string>& id2tok)->std::vector<int64_t> {
        std::unordered_map<std::string,int64_t> tok2id;
        for (int64_t i=0; i<(int64_t)id2tok.size(); ++i) if (!id2tok[(size_t)i].empty()) tok2id[id2tok[(size_t)i]] = i;
        auto find_tok = [&](const std::string& t)->int64_t { auto it = tok2id.find(t); return (it==tok2id.end()? -1 : it->second); };
        std::vector<int64_t> ids;
        auto push_if = [&](const std::string& t){ int64_t id = find_tok(t); if (id>=0) ids.push_back(id); };
        push_if("ĠQuestion"); push_if(":");
        std::istringstream iss(question); std::string w;
        while (iss >> w) {
            int64_t id = -1;
        ...
        if (question.find('?') != std::string::npos) { int64_t qid = find_tok("?"); if (qid >= 0) ids.push_back(qid); }
        push_if("ĠAnswer"); push_if(":");
        return ids;
    };
    auto GetInputDTypeByName = [&](OrtSession* s, const std::string& name, ONNXTensorElementDataType& out)->bool {
        size_t nin=0; ort_->SessionGetInputCount(s, &nin);
        OrtAllocator* alloc=nullptr; ort_->GetAllocatorWithDefaultOptions(&alloc);
        for (size_t i=0;i<nin;++i){
            char* nm=nullptr; if (!ort_->SessionGetInputName(s,i,alloc,&nm) && nm){
                std::string got(nm); alloc->Free(alloc,nm);
                if (got == name){
    ...
    // ===== Image path (from capture) =====
    if (image_path.empty()) {
        onnx_status_message_ += "No image path provided.\n"; clear_busy(); return;
    }
    if (!FileExists(image_path)) {
        onnx_status_message_ += ("Image not found: " + image_path + "\n"); clear_busy(); return;
    }

    // ===== Load vocab =====
    std::vector<std::string> id2tok;
    if (!FileExists(vocab_path) || !LoadGpt2VocabIdToTokenLocal(vocab_path, id2tok)) {
        onnx_status_message_ += "vocab.json missing or invalid.\n"; clear_busy(); return;
    }

    // ===== Start/EOS (defaults) =====
    int64_t start_id = 50256;
    int64_t eos_id   = 50256;
    if (FileExists(genconf_path)) {
        std::string g; if (ReadFile(genconf_path, g)) {
            auto pull_int = [&](const char* key, int64_t& out){
                size_t k = g.find(key); if (k==std::string::npos) return;
                k = g.find(':', k); if (k==std::string::npos) return; ++k;
                while (k<g.size() && (g[k]==' '||g[k]=='\t')) ++k;
                bool neg=false; if (k<g.size() && (g[k]=='-'||g[k]=='+')) { neg=(g[k]=='-'); ++k; }
                long long v=0; while (k<g.size() && g[k]>='0' && g[k]<='9') { v = v*10 + (g[k]-'0'); ++k; }
                out = neg? -v : v;
            };
    ...
    if (OrtStatus* st = ort_->CreateSession(env, enc_path.c_str(), so, &encoder_session_)) {
        onnx_status_message_ += "Encoder load failed.\n"; ort_->ReleaseStatus(st);
    } else onnx_status_message_ += "Encoder loaded.\n";
    if (OrtStatus* st = ort_->CreateSession(env, dec_path.c_str(), so, &decoder_session_)) {
    ...
    std::vector<float> pixel_values; int W=0,H=0;
    if (!LoadImageCHW224F32(image_path, pixel_values, W, H)) {
        onnx_status_message_ += ("Failed to load image: " + image_path + "\n");
        ort_->ReleaseSessionOptions(so); ort_->ReleaseEnv(env); clear_busy(); return;
    }
    ...
    {
        size_t nout = 0; ort_->SessionGetOutputCount(encoder_session_, &nout);
        if (nout > 0) {
            OrtAllocator* alloc = nullptr; ort_->GetAllocatorWithDefaultOptions(&alloc);
            char* nm = nullptr;
            if (!ort_->SessionGetOutputName(encoder_session_, 0, alloc, &nm) && nm) {
                enc_out_name = nm; alloc->Free(alloc, nm);
            }
        }
    }
    ...
    ort_->CreateTensorWithDataAsOrtValue(mi, pixel_values.data(), pixel_values.size()*sizeof(float),
    ...
    if (OrtStatus* st = ort_->Run(encoder_session_, nullptr, enc_in_names, &enc_input, 1, enc_out_names, 1, &enc_output)) {
        onnx_status_message_ += "Encoder run failed.\n"; ort_->ReleaseStatus(st);
    } else onnx_status_message_ += "Encoder OK.\n";
    ...
    if (enc_output) {
        ...
        float* p = nullptr; ort_->GetTensorMutableData(enc_output, (void**)&p);
    ...
    if (enc_feat.empty()) {
        ...
        if (so)  ort_->ReleaseSessionOptions(so);
        if (env) ort_->ReleaseEnv(env);
        clear_busy(); return;
    }
    ...
    { // inputs
        size_t nin = 0; ort_->SessionGetInputCount(decoder_session_, &nin);
        OrtAllocator* alloc = nullptr; ort_->GetAllocatorWithDefaultOptions(&alloc);
        for (size_t i=0; i<nin; ++i) {
            char* nm = nullptr;
            if (!ort_->SessionGetInputName(decoder_session_, i, alloc, &nm) && nm) {
                std::string s(nm); alloc->Free(alloc, nm);
                if (s.find("input_ids") != std::string::npos) name_input_ids = s;
                         ...
                         s.find("encoder_out")          !=std::string::npos) name_enc_hs = s;
    ...
    { // outputs
        size_t nout = 0; ort_->SessionGetOutputCount(decoder_session_, &nout);
        OrtAllocator* alloc = nullptr; ort_->GetAllocatorWithDefaultOptions(&alloc);
        if (nout > 0) {
            char* nm = nullptr;
            if (!ort_->SessionGetOutputName(decoder_session_, 0, alloc, &nm) && nm) {
                name_logits = nm; alloc->Free(alloc, nm);
            }
        }
    }
    ...
    {
        auto q_ids = TokenizeQuestion(question_text.empty() ? std::string("What is in the image?") : question_text, id2tok);
    ...
    if (ids_elem == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32) {
        ...
        ort_->CreateTensorWithDataAsOrtValue(mi, input_ids.data(), input_ids.size()*sizeof(int64_t),
    ...
    for (int step=0; step<max_steps; ++step) {
        ...
        size_t feed_count = 0;
        feed_names[feed_count]=name_input_ids.c_str(); feed_vals[feed_count]=t_ids; ++feed_count;
        feed_names[feed_count]=name_enc_hs.c_str();    feed_vals[feed_count]=t_hs;  ++feed_count;
        if (t_attn)     { feed_names[feed_count]=name_attn.c_str();     feed_vals[feed_count]=t_attn;     ++feed_count; }
        if (t_enc_attn) { feed_names[feed_count]=name_enc_attn.c_str(); feed_vals[feed_count]=t_enc_attn; ++feed_count; }
        ...
        if (logits && lshape.size()==3 && lshape[1]>0 && lshape[2]>0) {
            ...
            int64_t best_i = 0; float best_v = last[0];
            for (int64_t i=1;i<vocab;++i) if (last[i] > best_v) { best_v = last[i]; best_i = i; }
            if (best_i == eos_id && step < min_new_tokens) {
                int64_t second = best_i; float second_v = -1e30f;
                for (int64_t i=0;i<vocab;++i) { if (i==eos_id) continue; if (last[i] > second_v) { second_v=last[i]; second=i; } }
                next_id = second;
            } else next_id = best_i;
        }
        if (ti) ort_->ReleaseTensorTypeAndShapeInfo(ti);
        ...
        if (ids_elem == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32) {
            std::vector<int32_t> ids32(input_ids.begin(), input_ids.end());
            ort_->CreateTensorWithDataAsOrtValue(mi, ids32.data(), ids32.size()*sizeof(int32_t),
                                                 ids_shape2, 2, ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32, &t_ids);
        } else {
            ort_->CreateTensorWithDataAsOrtValue(mi, input_ids.data(), input_ids.size()*sizeof(int64_t),
                                                 ids_shape2, 2, ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64, &t_ids);
        }
        if (t_attn) {
            ONNXTensorElementDataType elem = ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64;
            ...
            if (elem == ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32) {
                ...
                ort_->ReleaseValue(t_attn); t_attn=nullptr;
                ort_->CreateTensorWithDataAsOrtValue(mi, ones.data(), ones.size()*sizeof(int32_t),
                                                     ids_shape2, 2, ONNX_TENSOR_ELEMENT_DATA_TYPE_INT32, &t_attn);
            } else {
                std::vector<int64_t> ones((size_t)new_len, 1);
                ort_->ReleaseValue(t_attn); t_attn=nullptr;
                ort_->CreateTensorWithDataAsOrtValue(mi, ones.data(), ones.size()*sizeof(int64_t),
                                                     ids_shape2, 2, ONNX_TENSOR_ELEMENT_DATA_TYPE_INT64, &t_attn);
            }
        }
    }

    onnx_status_message_ += "Q: " + (question_text.empty()? std::string("What is in the image?") : question_text) + "\n";
    onnx_status_message_ += "Generated token IDs (greedy): ";
    for (size_t i=0;i<generated.size();++i) { onnx_status_message_ += std::to_string(generated[i]); if (i+1<generated.size()) onnx_status_message_ += ", "; }
    onnx_status_message_ += "\n";
    std::string decoded = SimpleDecodeGpt2Local(generated, id2tok);
    onnx_status_message_ += "Answer: " + (decoded.empty() ? std::string("(empty)") : decoded) + "\n";
    ...
    onnx_status_message_ += "ONNX VQA complete.\n";
    clear_busy();
}
...
void CameraMixedRealityApp::Speak(const std::string& text) {
    ...
    JNIEnv* env = nullptr; app_state_->activity->vm->AttachCurrentThread(&env, nullptr);
    if (!env) return;
    jmethodID m = env->GetMethodID(g_VoiceBridgeCls, "speak", "(Ljava/lang/String;)V");
    if (!m) { ALOGE("speak(String) not found"); return; }
    jstring jtext = env->NewStringUTF(text.c_str());
    env->CallVoidMethod(g_VoiceBridgeObj, m, jtext);
